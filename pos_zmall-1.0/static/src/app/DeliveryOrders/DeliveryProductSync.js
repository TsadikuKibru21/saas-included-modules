...
export class DeliveryProductSync extends Component {
...
Syncsetup(){
    ...
    console.log(super.productsToDisplay);
    console.log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
    console.log("productswidget is loaded");

    let posProducts_names= [];
    let posProducts = [];


    if (this.productsToDisplay && this.productsToDisplay.length !== 0 && this.pos.posHasValidProduct()) {
        this.productsToDisplay.forEach(product => {
            posProducts.push(product);
            posProducts_names.push(product.display_name);
            ...
            //     imageUrl: this.pos.show_product_images && product.getImageUrl(),
            //     onClick: () => this.pos.addProductToCurrentOrder(product),
            //     productInfo: true,
            //     onProductInfoClick: () => this.onProductInfoClick(product)
            // });
    
            // // Render the ProductCard instance
            // // Note: You need to replace 'container' with the actual container where you want to render the ProductCard
    ...
    const products2 = JSON.parse(window.localStorage.getItem("liveproducts"));
    let  products2Names = [];
    if (products2 && products2.length){
        products2Names = products2.map(product => product.name);
    ...
    if (posProducts &&  posProducts.length){
        posProducts.forEach(posProduct => {
            console.log(products2Names.includes(posProduct.name));
            if(!products2Names.includes(posProduct.name)){
                
                console.log("pos product"+ posProduct);
                console.log("pos product id"+posProduct.id);
                console.log("pos product price"+posProduct.getFormattedUnitPrice());
                console.log("this.pos.config_id"+this.pos.config.id);
                let product_price = posProduct.getFormattedUnitPrice();
                let numericString = product_price.replace(/[^\d.]/g, '');
                
                // create product
                console.log("create"+posProduct.display_name);

                let storeId = window.localStorage.getItem("store_id");
                let serverToken = window.localStorage.getItem("server_token");
                // fixmebini hardcoded name
                let requestData = {
                ...
                let  config_id =  this.pos.config.id;
                

                
                jsonrpc('/web/dataset/call_kw/pos.config/add_item/',{
                    model: 'pos.config',
                    method: 'add_item',
                    ...
                    console.log("Value after the request: ", JSON.stringify(values)); // Convert the object to a string and then log it
                    // Now, check the structure of the values object and access the appropriate property containing the string you want to match
                    let stringValue = ""; // Update this with the actual property containing the string
                    let match = null;
                    if (typeof values === 'string') {
                        let match = values.match(/\((\d+),\)/);
                        // Rest of your code using match
                    } else {
                        console.log("Values is not a string. It's of type:", typeof values);
                    }
                
                    if (match) {
                        let id = match[1];
                        // Get the existing mapping from local storage
                        let mapping = JSON.parse(localStorage.getItem("mapping") || "{}");
                        // Update the mapping
                        mapping[posProduct.name] = id;
                        // Store the updated mapping back to local storage
                        window.localStorage.setItem("mapping", JSON.stringify(mapping));
                    }
                    // productMapper[posProduct.item_id] = values;
            ...
            console.log(posProduct.name);
            console.log(posProduct.price);
        });
    ...
    async handleaddposorder (event) {
        ...
        console.log("========================");
        console.log(event);
        for (prod of event.cart_items){
            console.log("././././././/./././././/./")
            console.log(prod);

        }
        /*
        {zmall_order_id: '6633ab0f664838728ae08a20', store_id: '6527e54f447f424c612e9910', unique_id: 226737, created_at: '2024-05-02T15:02:39.590Z', order_status: 5, …}
cart_items
: 
Array(1)
0
...
"6633ab0f664838728ae08a20"
    ...
    async addposorder(data,products=[]) {
        try {
            let self = this;
            self.setLoading(true);
            ...
            console.log(" value inside add_pos_order" + value)
            self.setLoading(false);
            console.log("======================= setOrdersStatus value =======================")
            console.log(value)
            console.log("======================= setOrdersStatus value =======================")
...
DeliveryProductSync.template = "pos_etta.DeliveryProductSync";
registry.category('pos_screens').add('DeliveryProductSync', DeliveryProductSync);
