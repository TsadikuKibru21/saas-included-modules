...
class DockerComposeTemplate(models.Model):
    ...
    def _default_template_odoo_conf(self):
        ...
        concurrent_users = 5
        cpu_cores = 2.0
        total_ram_mb = 2.0 * 1024
        
        # Worker calculation based on purchased_user, cpu_limit, and memory_limit
        max_cpu_workers = int(cpu_cores * 2) + 1
        min_workers_needed = max(1, concurrent_users // 6 + (1 if concurrent_users % 6 > 0 else 0))
        ram_per_worker_mb = (0.8 * 150) + (0.2 * 1024)  # ≈ 324.8MB per worker
        max_ram_workers = int(total_ram_mb * 0.7 / ram_per_worker_mb)
        workers = max(min_workers_needed, min(max_cpu_workers, max_ram_workers))
        cron_workers = 1

        # Adjust memory limits
        ram_per_worker_hard_mb = min(800, total_ram_mb * 0.7 / (workers + cron_workers))
        limit_memory_hard = int(ram_per_worker_hard_mb * 1024 * 1024)
        limit_memory_soft = int(limit_memory_hard * 0.75)

        # Append worker and resource settings to base_conf
    ...
    is_result_dc_body = fields.Boolean(string="Result docker compose")
      # Add this field for user input
    storage_limit_gb = fields.Float(string="Default Storage Limit (GB)", default=2.0)
    cpu_limit = fields.Float(string="Default CPU Limit", default=2.0)
    network_limit = fields.Float(string="Default Network Limit (Mbit/sec)", default=2.0)
    memory_limit = fields.Float(string='Memory Limit(Ram)', default=2)
